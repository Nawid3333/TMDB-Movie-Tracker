"""Terminal helpers: the colour gate, semantic styles, display width, boxes.

All six sibling repos share one colour vocabulary; see the marker table in
`_LEAD_RULES` and the `cprint` docstring below. Modules opt in at the import:

    from src.ui.term import cinput as input, cprint as print

so every existing ``print`` and ``input`` call keeps its shape and only gains
colour. Only a *line-leading* arrow is treated as a heading -- an arrow used
mid-sentence ("12/24 -> 24/24") is left alone.

Anything the markers cannot express calls a style function directly: `err`,
`danger`, `critical`, `warn`, `alert`, `ok`, `success`, `step`, `accent`,
`title`, `bold`, `dim`. `Style` holds the raw codes behind those -- reach for it only
where none of them says what is meant, and give a repeated combination a name
here instead.
"""
